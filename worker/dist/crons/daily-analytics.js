...
exports.aggregateDailyAnalytics = aggregateDailyAnalytics;
...
async function aggregateDailyAnalytics() {
    ...
    const liveCampaigns = await db_1.db.campaign.findMany({
    ...
    for (const campaign of liveCampaigns) {
        try {
            ...
            const clicks = await db_1.db.clickEvent.count({
                where: {
            ...
            const conversions = await db_1.db.conversionEvent.count({
                where: {
                    campaignId: campaign.id,
                    createdAt: { gte: yesterday, lt: today },
                    isVerified: true,
            ...
            const viewSnapshots = await db_1.db.viewSnapshot.aggregate({
                where: {
                    campaignId: campaign.id,
            ...
            const transactions = await db_1.db.transaction.aggregate({
                where: {
                    participation: {
                        campaignId: campaign.id,
                    },
            ...
            await db_1.db.campaignDailyAnalytics.upsert({
                where: {
                    campaignId_date: {
                        campaignId: campaign.id,
                        date: yesterday,
    ...
    try {
        const totalGMV = await db_1.db.transaction.aggregate({
            where: {
                createdAt: { gte: yesterday, lt: today },
                type: "CAMPAIGN_FUND",
                status: "COMPLETED",
        ...
        const totalPayouts = await db_1.db.payout.aggregate({
            where: {
        ...
        const totalRevenue = await db_1.db.transaction.aggregate({
            where: {
                createdAt: { gte: yesterday, lt: today },
                type: "PLATFORM_FEE",
                status: "COMPLETED",
        ...
        const fraudFlags = await db_1.db.fraudFlag.count({
            where: {
                createdAt: { gte: yesterday, lt: today },
            },
        });
        const totalFlags = await db_1.db.fraudFlag.count({
            where: { createdAt: { lt: today } },
        });
        const confirmedFraud = await db_1.db.fraudFlag.count({
            where: {
                status: "CONFIRMED",
                createdAt: { lt: today },
            },
        });
        await db_1.db.platformDailyAnalytics.upsert({
...
//# sourceMappingURL=daily-analytics.js.map