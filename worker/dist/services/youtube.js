...
exports.getVideoMetrics = getVideoMetrics;
const YT_API_BASE = "https://www.googleapis.com/youtube/v3";
const YT_ANALYTICS_BASE = "https://youtubeanalytics.googleapis.com/v2";
async function getVideoMetrics(videoUrl, tokenPayload, creatorId) {
...
async function fetchAnalyticsMetrics(videoId, accessToken) {
    try {
...
async function fetchPublicMetrics(videoId) {
    ...
    try {
        const response = await fetch(`${YT_API_BASE}/videos?part=statistics&id=${videoId}&key=${apiKey}`);
        if (!response.ok) {
