 ...
 */
async function calculateEarnings(campaignId, creatorId) {
    const campaign = await db_1.db.campaign.findUnique({
        where: { id: campaignId },
        select: {
            type: true,
 ...
 */
async function updateMetrics(campaignId, creatorId, updates) {
    const earnings = await calculateEarnings(campaignId, creatorId);
    await db_1.db.campaignMetrics.upsert({
        where: {
            campaignId_creatorId: { campaignId, creatorId },
        },
        create: {
