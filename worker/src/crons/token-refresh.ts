...
export function startTokenRefreshCron() {
  ...
  cron.schedule('0 2 * * *', async () => {
    console.log(
      '[token-refresh] Starting Instagram long-lived token refresh job...'
    );
    try {
...
async function refreshInstagramTokens() {
  ...
  for (const creator of creators) {
    try {
      ...
      if (!plainToken) {
        console.warn(
          `[token-refresh] Could not decrypt token for user ${creator.userId} — skipping`
        );
        failureCount++;
        continue;
      }
      ...
      if (!response.ok) {
        ...
        console.warn(
          `[token-refresh] Failed to refresh FB token for user ${creator.userId}: ${response.status} ${errBody}`
        );
        failureCount++;
        continue;
      }
      ...
      console.error(
        `[token-refresh] Error refreshing token for ${creator.userId}`,
        err
      );
      failureCount++;
    }
  ...
  console.log(
    `[token-refresh] Job complete. Success: ${successCount}, Failures: ${failureCount}`
  );
}
