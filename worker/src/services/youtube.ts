...
export async function getVideoMetrics(
  ...
  if (accessToken) {
    if (
      ...
      try {
        ...
        if (refreshResponse.ok) {
          ...
          console.log(
            `[youtube] Refreshed YT access token for creator ${creatorId}`
          );
        }
      } catch (e) {
    ...
    const analyticsResult = await fetchAnalyticsMetrics(
      videoId,
    ...
    console.warn(
      `[youtube] Analytics API failed for ${videoId}, falling back to public API`
    );
  }
...
async function fetchAnalyticsMetrics(
  ...
  try {
    ...
    url.searchParams.set(
      'metrics',
      'views,likes,comments,estimatedMinutesWatched,averageViewPercentage'
    );
...
async function fetchPublicMetrics(videoId: string): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[youtube] Public API error: ${response.status} ${response.statusText}`
      );
      return null;
    }
