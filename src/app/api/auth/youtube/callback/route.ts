...
export async function GET(request: Request) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?error=youtube_auth_failed`
    );
  }

  if (!code) {
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?error=missing_code`
    );
  }
  ...
  if (!clientId || !clientSecret) {
    ...
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?error=server_configuration_error`
    );
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?error=token_exchange_failed`
      );
    }
    ...
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?success=youtube_connected`
    );
  } catch (error) {
    console.error('Error during YouTube OAuth callback:', error);
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/creator/dashboard?error=internal_server_error`
    );
  }
}
