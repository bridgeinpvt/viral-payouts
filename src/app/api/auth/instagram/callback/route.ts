...
export async function GET(request: Request) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      `${baseUrl}/creator/dashboard?error=instagram_auth_failed`
    );
  }

  if (!code) {
    return NextResponse.redirect(
      `${baseUrl}/creator/dashboard?error=missing_code`
    );
  }
  ...
  if (!clientId || !clientSecret) {
    ...
    return NextResponse.redirect(
      `${baseUrl}/creator/dashboard?error=server_configuration_error`
    );
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(
        `${baseUrl}/creator/dashboard?error=token_exchange_failed`
      );
    }
    ...
    if (!longLivedResponse.ok) {
      ...
      return NextResponse.redirect(
        `${baseUrl}/creator/dashboard?error=long_lived_token_failed`
      );
    }
      ...
      if (pagesResponse.ok) {
        ...
        if (firstPage?.id) {
          ...
          if (igAccountResponse.ok) {
            ...
            instagramUserId =
              igAccountData?.instagram_business_account?.id ?? null;
          }
        }
      }
    } catch (e) {
    ...
    return NextResponse.redirect(
      `${baseUrl}/creator/dashboard?success=instagram_connected`
    );
  } catch (error) {
    console.error('Error during Instagram OAuth callback:', error);
    return NextResponse.redirect(
      `${baseUrl}/creator/dashboard?error=internal_server_error`
    );
  }
}
